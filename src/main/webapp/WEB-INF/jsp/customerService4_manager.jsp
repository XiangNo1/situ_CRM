...
$(function(){
	/*展示数据的datagrid表格*/
	$("#datagrid").datagrid({
		url:'${ctx}/customerService4/findAll.action',
		method:'get',
		fit:true,
		singleSelect:false,
		toolbar:'#toolbar',
...
function openAddDialog() {
	...
	url = "${ctx}/customerService4/addCustomerService4.action";
	$('#form').form("clear");
...
function openUpdateDialog() {
	...
	url = "${ctx}/customerService4/updateCustomerService4.action";
	$('#form').form("load", row);
}
...
function doDelete() {
	...
	$.messager.confirm('确认','您确认想要删除记录吗？',function(r){    
	    if (r){    
	    	$.post(
					"${ctx}/customerService4/delete.action",
					{ids:ids}, 
					function(data) {
						$.messager.progress('close');	// 如果表单是无效的则隐藏进度条
						if(data.status == 0) {
...
<body>
	...
	<!-- toolbar 开始 -->
	<div id="toolbar">
		<a class="easyui-linkbutton" href="javascript:openUpdateDialog()" iconCls="icon-fwfk">客户服务反馈</a>
	</div>
	<!-- toolbar 结束 -->
	...
	<div id="dialog" class="easyui-dialog" closed="true" modal="true"
		...
		<form action="" id="form" method="post">
			...
			<table cellspacing="8px">
	   			...
	   			<tr>
		   			<td>处理结果：</td>
		   			<td><input type="text" id="serviceDealResult" name="serviceDealResult" required="true" /></td>
					<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
					<td>客户满意度：</td>
