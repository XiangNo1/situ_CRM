...
$(function(){
	/*展示数据的datagrid表格*/
	$("#datagrid").datagrid({
		url:'${ctx}/customerService3/findAll.action',
		method:'get',
		fit:true,
		singleSelect:false,
		toolbar:'#toolbar',
...
function openAddDialog() {
	...
	url = "${ctx}/customerService3/addCustomerService3.action";
	$('#form').form("clear");
...
function openUpdateDialog() {
	...
	url = "${ctx}/customerService3/updateCustomerService3.action";
	$('#form').form("load", row);
}
...
function doDelete() {
	...
	$.messager.confirm('确认','您确认想要删除记录吗？',function(r){    
	    if (r){    
	    	$.post(
					"${ctx}/customerService3/delete.action",
					{ids:ids}, 
					function(data) {
						$.messager.progress('close');	// 如果表单是无效的则隐藏进度条
						if(data.status == 0) {
...
<body>
	...
	<!-- toolbar 开始 -->
	<div id="toolbar">
		<a class="easyui-linkbutton" href="javascript:openUpdateDialog()" iconCls="icon-fwcl">处理</a>
	</div>
	<!-- toolbar 结束 -->
		...
		<form action="" id="form" method="post">
			...
			<table cellspacing="8px">
	   			...
	   			<tr>
		   			<td>处理人：</td>
		   			<td>
		   				<input class="easyui-combobox" id="serviceDealPeople" name="serviceDealPeople" data-options="panelHeight:'auto',editable:false,valueField:'trueName',textField:'trueName',url:'${ctx}/user/getCustomerManagerList.action'"/>
		   			</td>
		   			<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
		   			<td>处理时间：</td>
